...
export interface PayableReportEntry {
  contactId: string;
  contactName: string;
  email: string | null;
...
export async function getPayableReport(
  ...
  // 1. Opening balance: invoices issued before startDate - payments before startDate - returns before startDate
  const openingInvoices = await prisma.purchaseInvoice.findMany({
    where: {
  ...
  const openingPayments = await prisma.purchasePayment.findMany({
    where: {
  ...
  const openingReturns = await prisma.purchaseReturn.findMany({
    where: {
  ...
  const periodReturns = await prisma.purchaseReturn.findMany({
    where: {
  ...
  const result: PayableReportEntry[] = [];
