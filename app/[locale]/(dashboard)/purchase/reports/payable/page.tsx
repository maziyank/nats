...
export default function PayableReportPage() {
  ...
  const {
    ...
    queryKey: ["purchase-payable", startDate, endDate],
    queryFn: async () => {
      return await getPayableReport(
        new Date(startDate),
        new Date(endDate)
      );
    },
    ...
    <div className="flex flex-1 flex-col gap-2 p-4 pt-0">
      <div className="flex flex-col gap-4">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-lg font-bold">{t("reports_payable_heading")}</h1>
            <p className="text-sm text-muted-foreground">
              {t("reports_payable_subheading")}
            </p>
          </div>
