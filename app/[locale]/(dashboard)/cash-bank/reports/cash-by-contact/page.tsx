...
import {
  getCashByContactReport,
  type CashByContactResult,
} from "../actions";
...
export default function CashByContactReportPage() {
  ...
  const {
    ...
    isLoading: loading,
    refetch,
  } = useQuery<CashByContactResult>({
    queryKey: ["cash-bank-by-contact", startDate, endDate],
    queryFn: async () =>
      getCashByContactReport(new Date(startDate), new Date(endDate)),
  });
      ...
      <div className="flex flex-col gap-4">
        <div className="mb-2">
          <h1 className="text-lg font-bold">
            {t("reports_cash_by_contact_heading")}
          </h1>
        </div>
        ...
        {!loading && report && report.entries.length > 0 && (
          <div className="rounded-md border">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead>{t("reports_col_contact")}</TableHead>
              ...
              <TableBody>
                {report.entries.map((entry, idx) => (
                  <TableRow key={entry.contactId ?? `none-${idx}`}>
