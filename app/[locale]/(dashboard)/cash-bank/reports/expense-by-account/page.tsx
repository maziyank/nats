...
export default function ExpenseByAccountReportPage() {
  ...
  const {
    ...
    queryKey: ["cash-bank-expense-by-account", startDate, endDate],
    queryFn: async () =>
      getAllocationByAccountReport(
        new Date(startDate),
        new Date(endDate),
  ...
  const { isExporting, exportingFormat, exportCsv, exportExcel } =
    useReportExport<Record<string, unknown>>({
      ...
      filename: () => `expense-by-account-${startDate}-${endDate}`,
      sheetName: "Expense by Account",
      ...
      <div className="flex flex-col gap-4">
        <div className="mb-2">
          <h1 className="text-lg font-bold">
            {t("reports_expense_by_account_heading")}
          </h1>
        </div>

        <div className="flex justify-between items-center">
          <p className="text-sm text-muted-foreground">
            {t("reports_expense_by_account_subheading")}
          </p>
