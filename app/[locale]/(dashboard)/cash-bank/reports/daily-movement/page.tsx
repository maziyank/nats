...
export default function DailyCashMovementReportPage() {
  ...
  // Default to current month for daily report
  ...
  const {
    ...
    isLoading: loading,
    refetch,
  } = useQuery<DailyMovementResult>({
    queryKey: ["cash-bank-daily-movement", startDate, endDate],
    queryFn: async () =>
      getDailyCashMovementReport(new Date(startDate), new Date(endDate)),
  });
      ...
      <div className="flex flex-col gap-4">
        <div className="mb-2">
          <h1 className="text-lg font-bold">
            {t("reports_daily_movement_heading")}
          </h1>
        </div>
